...
def main() -> None:
    ...
    AUDIO_SIGNAL = load_audio_example(audio=AudioExample.WALKING_GRASS)
    frame_block = AUDIO_SIGNAL.readframes(CHUNK)
    ...
    match example_mode:
        case "linear_traj":
            cpa = np.array([1.0, 0.0, 0.0]) # closest point (left Y pos, in front X pos, up Z pos)
            ...
            points = linear_traj.get_points(distances=distances)
        case "circular_traj" | "parabolic_traj":
            ...
            if example_mode == "circular_traj":
                points = ParametricTrajectory.get_circular_points(omega=omega, t=times, radius=distances_circular, start_elevation=0.0, vertical_vel=vertical_vel)
            else:
                points = ParametricTrajectory.get_parabolic_points(total_distance=MAX_DISTANCE, total_duration=nblocks * CHUNK / SR, max_elevation=30, t=times)
